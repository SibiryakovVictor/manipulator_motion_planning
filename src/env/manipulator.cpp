...
const env::OBB & ManipulatorController::getBodyCurLink() const
{
...
uint16_t ManipulatorController::setConfigCurrentLink( const float & config )
{
...
const env::OBB & ManipulatorController::getBody( uint16_t id ) const
{
	if ( id < biasGroupId )
	{
...
const env::OBB & ManipulatorController::getBodyGroup( uint16_t groupId ) const
{
...
uint16_t ManipulatorController::getFirstLinkComplex( uint16_t complex ) const
{
...
env::Orient ManipulatorController::appendInitOrient( uint16_t linkId, 
...
env::OBB::Sizes ManipulatorController::calcComplexSize( uint16_t linkId ) const
{
...
env::Position ManipulatorController::calcComplexPos( uint16_t linkId ) const
{
...
void ManipulatorController::defineOrientDependencies()
{
		...
		groupId++ )
	{
		...
		while ( searchPos < manip::links_amount )
		{
			...
			m_linkOrientDep[ depPos ] = { groupId, searchPos };
...
void ManipulatorController::defineFirstLinkComplex()
{
	uint16_t linkPos = 0;

	std::for_each( m_linkType, m_linkType + manip::links_amount,
		[ this, &linkPos ]( uint16_t linkType )
		{
			if ( linkType > 1 )
			{
				m_complexFirstLink[ linkPos ] = linkPos;
			}
